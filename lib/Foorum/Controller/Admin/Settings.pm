...
sub default : Private {
    ...
    # function on
    my $maintain = $params{maintain};     $maintain = 0 if ($maintain != 1);
    my $register = $params{register};     $register = 1 if ($register != 0);
    my $activation = $params{activation}; $activation = 1 if ($activation != 0);
    $yaml->{function_on} = {
    ...
    $yaml->{per_day} = {
        most_deletion_topic => $most_deletion_topic || 5
    };
